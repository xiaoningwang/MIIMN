...
class DynamicLSTM(nn.Module):
    ...
    def forward(self, x, x_len):
        ...
        """sort"""
        # x_len = x_len.cpu() 
        # if using cpu, use these:
        x_sort_idx = np.argsort(-x_len.cpu())  # x_sort_idx = np.argsort(-x_len)
        x_unsort_idx = torch.LongTensor(np.argsort(x_sort_idx),)
        
        # if using gpu, use these:
        # x_sort_idx = np.argsort(-x_len.cpu())
        # x_unsort_idx = torch.tensor(np.argsort(x_sort_idx), device='cuda')
        # x_unsort_idx.long()
